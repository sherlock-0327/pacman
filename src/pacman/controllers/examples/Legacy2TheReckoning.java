...
public class Legacy2TheReckoning extends Controller<EnumMap<GHOST, MOVE>> {
		...
		for (GHOST ghost : GHOST.values()) {
			if (game.doesGhostRequireAction(ghost)) {
				...
				// if edible or Ms Pac-Man is close to power pill, move away from Ms Pac-Man
					...
					myMoves.put(ghost, game.getApproximateNextMoveAwayFromTarget(currentIndex, pacmanIndex,
							game.getGhostLastMoveMade(ghost), DM.PATH)); // move away from ms pacman
				// else go towards Ms Pac-Man
				else
					myMoves.put(ghost, game.getApproximateNextMoveTowardsTarget(currentIndex, pacmanIndex,
							game.getGhostLastMoveMade(ghost), DM.PATH)); // go towards ms pacman
			}
		}
	...
	private boolean isCrowded(Game game) {
		...
		for (int i = 0; i < ghosts.length - 1; i++)
			for (int j = i + 1; j < ghosts.length; j++)
				distance += game.getShortestPathDistance(game.getGhostCurrentNodeIndex(ghosts[i]),
	...
	private MOVE getRetreatActions(Game game, GHOST ghost) {
		...
		if (game.getGhostEdibleTime(ghost) == 0
			...
			return game.getApproximateNextMoveTowardsTarget(currentIndex, pacManIndex, game.getGhostLastMoveMade(ghost),
					DM.PATH);
		else
